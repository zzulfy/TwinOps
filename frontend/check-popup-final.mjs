    ...
    try {
        ...
        if (labels.length > 0) {
            ...
            if (popup) {
                ...
                // 截图保存（包含弹窗）
                await page.screenshot({ path: 'check-popup-final.png', fullPage: true });
                console.log('✅ 弹窗截图已保存到: check-popup-final.png');
                ...
                // 点击关闭按钮
                console.log('=== 点击关闭按钮 ===');
                const closeBtn = await page.$('.device-detail-panel .close-btn');
                if (closeBtn) {
                    ...
                    // 打印所有可能的关闭按钮
                    const allCloseButtons = await page.$$('*');
                    console.log('所有元素:', await page.evaluate(() => {
                        return Array.from(document.querySelectorAll('*')).map(el => {
                            return {
                                tagName: el.tagName,
                                className: el.className,
                ...
                await page.screenshot({ path: 'no-popup-final.png', fullPage: true });
                console.log('✅ 无弹窗截图已保存到: no-popup-final.png');
            ...
            await page.screenshot({ path: 'no-labels-final.png', fullPage: true });
            console.log('✅ 无设备标签截图已保存到: no-labels-final.png');
        }
