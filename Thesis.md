...
在数据质量方面，实际运维场景中遥测数据可能面临三类挑战：一是传感器偶发性故障导致的数据缺失或异常跳变[20]，二是不同设备因固件版本差异导致的采样频率不完全一致，三是设备重启期间产生的零值或空值数据。AERCA模型对输入数据的完整性有一定要求——缺失值会破坏滑动窗口的连续性，影响编码器系数矩阵的学习质量。本文在数据预处理阶段通过线性插值填补短时缺失（不超过3个连续时间步），对长时缺失则标记该设备暂不参与当次因果分析，从而保障输入数据的质量。
