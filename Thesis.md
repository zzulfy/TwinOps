...
领域层封装业务规则：设备聚合根管理、遥测查询与聚合、告警生命周期（新建→已解决）、报告幂等创建与状态流转（处理中→成功/失败）、加权Z-score特征工程和AERCA结果解析存储（详见第2.1.4节AERCA模型原理）。
...
第五步，RCA因果推理。特征工程（详见第2.1.4节加权正向Z-score特征工程方法）归一化为应力序列，组织为时间×设备矩阵调用RCA侧车。不可达时降级为LLM独立分析。
...
特征工程（buildWindow四步，原理详见第2.1.4节）：（1）筛选error优先于warning，取前10台；（2）每台查最近30条记录；（3）六项指标正向Z-score（标准差小时防除零，截断负值），按权重加权求和；（4）不足30点左填充，转置为时间×设备矩阵。
