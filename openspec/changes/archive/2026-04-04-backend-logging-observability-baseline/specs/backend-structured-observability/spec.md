...
- **THEN** backend logs for that request use the same request id
- **AND** the same id is available to downstream service and exception logs via context propagation

#### Scenario: Incoming request lacks request id
- **WHEN** an HTTP request does not include `X-Request-Id`
- **THEN** backend generates a new request id
- **AND** generated id is used consistently across logs for that request
...
- **THEN** logs do NOT contain raw passwords, API keys, or authorization tokens
- **AND** any sensitive value representation is redacted or omitted
