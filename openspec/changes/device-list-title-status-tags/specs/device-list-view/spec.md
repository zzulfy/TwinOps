...
The device module SHALL provide a searchable list-first experience for all devices and SHALL allow navigation from list items to focused single-device detail pages. The device list surface SHALL include alarm lifecycle operations so operators can resolve alarms in an operational context. Each device list item SHALL render title text as `deviceCode + device name` and SHALL render status as a dedicated tag row (`normal | warning | error`) instead of concatenated `deviceCode · status` text.
...
#### Scenario: Device list item displays title and status with new hierarchy
- **WHEN** device list item is rendered
- **THEN** title MUST display `deviceCode` followed by device name
- **AND** the row below MUST display only a status tag (`normal` or `warning` or `error`) without `deviceCode · status` text
