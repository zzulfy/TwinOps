...
#### Scenario: Alarm list keeps smooth motion under periodic rotation
- **WHEN** dashboard alarm panel runs continuous auto-scroll cycles
- **THEN** each cycle completes with smooth movement and without obvious frame stutter
- **AND** list content does not visibly jump during the transition between cycles
