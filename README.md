...
- Analysis：触发并查看聚合 AI 分析报告，并在启用 sidecar 时查看设备级 RCA 证据
...
- `causaltrace-rca/`：TwinOps RCA sidecar 与 AERCA 根因分析模块
- `data/`：SMD / MSDS 数据集
- `reports/`：截图与验证产物
- `openspec/`：需求与规格工件
...
- Analysis 采用聚合批处理：一次 trigger 只发布 1 条 Kafka job，并最终生成 1 条聚合报告（`deviceCode=AGGREGATED`）；启用 `twinops.analysis.rca` 后，consumer 会先组装设备级 stress time-window，调用 `causaltrace-rca` sidecar 生成 root-cause ranking / causal edges，再把结构化 RCA 结果和 LLM 文本一起写入报告
- LLM 调用默认允许 fallback 到本地 mock，避免演示环境因外部模型不可用而完全失败
- `causaltrace-rca/` 当前除了研究脚本外，还提供 FastAPI sidecar：`GET /health`、`POST /infer/device-rca`；后端 sidecar 不可用时自动 fallback 到 `llm_only`
...
### 5. 启动 RCA sidecar（可选，但设备级 RCA 需要）
