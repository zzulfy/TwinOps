...
### RocketMQ Local Setup (Required for Analysis Automation, non-Docker first)

Analysis automation uses scheduler + RocketMQ producer/consumer. Local verification requires NameServer and Broker.

#### Non-Docker startup (recommended)

```bash
# Install JDK17 first, then download RocketMQ binary package
...
docker run -d --name rmqbroker --link rmqnamesrv:namesrv \
  ...
  apache/rocketmq:5.3.2 sh mqbroker -n namesrv:9876 --enable-proxy
```

If RocketMQ is not available, automated analysis pipeline will not run correctly.

### RocketMQ Config Mapping

In `application.yml`, ensure:

- `rocketmq.name-server: 127.0.0.1:9876`
- `rocketmq.producer.group: twinops-analysis-producer`
...
### Analysis Report Workflow (Automated)

- Reports are generated by backend scheduled automation (half-day slots: `00:00` and `12:00`).
- Scheduler collects target fault devices and publishes analysis requests to RocketMQ topic `analysis.request`.
- Consumer processes messages, executes LLM analysis, and persists to `analysis_reports`.
- Idempotency key uses `deviceCode + half-day slot` to prevent duplicate effective report writes.
- Manual `POST /api/analysis/reports` submission is disabled for operator workflow.
...
### RocketMQ Automation Config

The above mapping is the runtime contract for scheduler/producer/consumer wiring.
