    ...
    default String generateReport(
        ...
        if (rootCauses == null || rootCauses.isEmpty()) {
            ...
            for (AnalysisRootCauseDto rc : rootCauses) {
                sb.append("- ").append(rc.deviceCode())
                    .append("（评分: ").append(String.format("%.2f", rc.score() == null ? 0.0 : rc.score()))
                    .append("，排名: #").append(rc.rank() == null ? "-" : rc.rank()).append("）\n");
            }
            sb.append("\n");
        }
        ...
        if (causalEdges == null || causalEdges.isEmpty()) {
            ...
            for (AnalysisCausalEdgeDto edge : causalEdges) {
                sb.append("- ").append(edge.fromDeviceCode())
                    .append(" → ").append(edge.toDeviceCode())
                    .append("（权重: ").append(String.format("%.2f", edge.weight() == null ? 0.0 : edge.weight())).append("）\n");
            }
            sb.append("\n");
        }
